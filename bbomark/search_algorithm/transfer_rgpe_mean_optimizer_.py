...
from bbomark.surrogate.rgpe import RGPE_mean_surrogate_
...
class SMBO(AbstractOptimizer, FeatureSpace_uniform):
    ...
    def __init__(self,
        ...
        self.acq_class = qNoisyExpectedImprovement
    ...
    def suggest(self, n_suggestions=1):
        ...
        if (self.trials.trials_num) < self.min_sample:
            ...
            for n in range(n_suggestions):
                surrogate = RGPE_mean_surrogate_(self.gps + [self.target_model],
                                                 torch.Tensor(self.weights))
                sampler_qnei = SobolQMCNormalSampler(num_samples=self.mc_samples)
                acq = self.acq_class(surrogate, X_baseline=-self.x.unsqueeze(0), sampler=sampler_qnei)
    ...
    def observe(self, x, y):
        ...
        y = torch.Tensor(self.trials.history_y).unsqueeze(-1)
        train_yvar = torch.full_like(y, self.noise_std ** 2)
        self.target_model = get_fitted_model(self.x, y, train_yvar)

        self.weights = self._get_weight(self.x, y.squeeze())
