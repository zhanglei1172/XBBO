...
class Node:
    ...
    def __init__(self, parent = None, sample_dims=0, split_dims=0, true_dims=0, reset_id = False, kernel_type = "rbf",cmaes_sigma_mult=1.,leaf_size=20, splitter_type='kmeans', split_metric='max',use_gpr=True, gamma_type = "auto", verbose = False,rng=np.random.RandomState(42),split_use_predict=False,**kwargs):
        ...
        self.uct           = 0
        self.rng = rng
        self.classifier    = Classifier( [], [], self.sample_dims, self.split_dims, self.true_dims, kernel_type,cmaes_sigma_mult,leaf_size, splitter_type, split_metric,use_gpr, gamma_type, verbose=verbose, rng=self.rng,split_use_predict=split_use_predict,**kwargs )
    ...
    def propose_samples_bo(self, latent_samples, f_samples, num_samples, path, lb, ub, samples):
        proposed_X = self.classifier.propose_samples_bo(latent_samples, f_samples, num_samples, path, lb, ub, samples)
        return proposed_X
        
    def propose_samples_rs(self, latent_samples, f_samples, num_samples, path, lb, ub, samples):
        proposed_X = self.classifier.propose_samples_rs(latent_samples, f_samples, num_samples, path, lb, ub, samples)
        return proposed_X
    ...
    def propose_samples_cmaes(self, num_samples, path, init_within_leaf, lb, ub):
        proposed_X = self.classifier.propose_samples_cmaes(num_samples, path, init_within_leaf, lb, ub)
        return proposed_X
    ...
    def __str__(self):
        ...
        upper_bound = np.around( np.max(self.classifier.sample_X, axis = 0), decimals=2 )
        lower_bound = np.around( np.min(self.classifier.sample_X, axis = 0), decimals=2 )
        boundary    = ''
