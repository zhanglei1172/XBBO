...
class AbstractBenchmark(object, metaclass=abc.ABCMeta):
    ...
    def _check_configuration(foo):
        ...
        def wrapper(self, configuration, **kwargs):
            if not isinstance(configuration, ConfigSpace.Configuration):
                try:
                    squirtle = configuration if isinstance(configuration, dict) else {k: configuration[i] for (i,k) in enumerate(self.configuration_space)}
    ...
    def __call__ (self, configuration, **kwargs):
        ...
        self.counter += 1
        return_dict = {Key.SUGGEST_INFO: kwargs}
        return_dict.update(kwargs)
        return_dict.update(self.objective_function(configuration, **kwargs))
        return return_dict
