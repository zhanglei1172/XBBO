...
from xbbo.search_algorithm.multi_fedility.utils.bracket_manager import ConfigGenerator, SHBracketManager
...
class HB(AbstractOptimizer):
    ...
    def check_stop(self, ):
        if self.round_recoder >= self.round_limit or self.learner_time_recoder >= self.learner_time_limit or self.total_time_recoder >= self.total_time_limit:
            return True
        else:
            return False
