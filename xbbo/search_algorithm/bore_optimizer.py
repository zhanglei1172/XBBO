...
class BORE(AbstractOptimizer):
    ...
    def suggest(self, n_suggestions=1):
        ...
        X_init = self.rng.uniform(low=self.bounds.lb, high=self.bounds.ub, size=(self.num_samples, self.dense_dimension))
        ...
        for n_ in range(n_suggestions):
            ...
            if self.num_starts > 0:
                ind = np.argpartition(f_init, kth=self.num_starts-1, axis=None)
                for i in range(self.num_starts):
                    x0 = X_init[ind[i]]
                    result = minimize(self.classifier.predict, x0=x0, method=self.method,
                                    jac=False, bounds=self.bounds, options=self.options)
            ...
            for res in results:
                if (res.success or res.status == 1) and res.fun < best_v:
                    config = DenseConfiguration.from_dense_array(self.space, res.x)
                    if not self.trials.is_contain(config):
                        best_config = config
